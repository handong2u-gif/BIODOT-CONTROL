...
async function searchProduct() {
    try {
        // Search for products containing '흑염소'
        const url = `${baseUrl}/finished_goods?product_name=ilike.*흑염소*&select=id,product_name,created_at,updated_at`;
        const res = await fetch(url, { headers });
        const data = await res.json();
        console.log('--- Search Results for "흑염소" ---');
